...
contract MCU is RewardsToken {
    ...
    uint256 public burnFee = 500; // 5%  
    uint256 public sellburnFee = 500; // 5%
    ...
    // Variables for Master Burn / Clearing House Burn
    uint256 private minETHForBurn =  1000000000000000000; //1 ETH
    bool private autoBurn = true; //Will automatically Burn tokens when contract balance gets above certain amount
    address public nutraceuticalFinaceContractAddres;
    ...
    event TokensBurned(address wallet, uint256 amount);
    ...
    constructor() ERC20("Master Burn", "MCU") payable {
    ...
    //transfer function goes here
     function _transfer(
        ...
        //This is to calculate how many tokens should be burned. This is separate from the 'calculateFees' function because this mixes all of it into 1 single fee amount. 
        ...
        if(autoBurn && address(this).balance >= minETHForBurn){
            swapEThForNutraTokens();
        }
    }
    ...
    ///clearing House Burn functions

    function setNutraAddress(address _nutraAddress) public onlyOwner{
        nutraceuticalFinaceContractAddres = _nutraAddress;
    }  


    function setMinETHForBurn(uint256 _ethMinForBurn) public onlyOwner{
        minETHForBurn = _ethMinForBurn;
    }
    ...
    function BurnNutraTokensWithETh() public payable onlyOwner{
        require((nutraceuticalFinaceContractAddres != 0x0000000000000000000000000000000000000000), "Contract Address has not been set");
        swapEThForNutraTokens();
    }

    function swapEThForNutraTokens() private { 
        //generate the uniswap pair path of token => Weth
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = nutraceuticalFinaceContractAddres;

        //make the swap
        uint256 amount = address(this).balance;
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, //function to set this number
            path, 
            address(this), 
            block.timestamp
            );
